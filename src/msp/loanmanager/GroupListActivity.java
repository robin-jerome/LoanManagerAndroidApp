	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (MainActivity.groups.size() == 0) {
			...
			for (int i = 0; i < MainActivity.groups.size(); i++) {
				...
				LayoutParams numberparams = new TableRow.LayoutParams(
						LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
				number.setLayoutParams(numberparams);
				number.setText(Integer.toString(MainActivity.groups.get(i)
						.getPersonsCount()));
				number.setTextColor(Color.BLACK);
				number.setGravity(Gravity.RIGHT);
				number.setTextSize(20);
				number.setPadding(20, 20, 20, 20);
				tr.addView(number);
				...
				tr.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						Intent intent;
						if (fromLoans) {
							intent = new Intent(GroupListActivity.this,
									GroupLoansActivity.class);
						} else {
							intent = new Intent(GroupListActivity.this,
									GroupDescriptionActivity.class);
						}
