...
public class AddGroupActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		TableLayout tl = (TableLayout) sv
				.findViewById(R.id.add_group_choose_persons_table);
		...
		for (int i = 0; i < MainActivity.persons.size(); i++) {
			...
			if (egroup != null
					&& egroup.isInGroup(MainActivity.persons.get(i).getId())) {
			...
			public void onClick(View v) {
				...
				if (id == 0) {
					if (MainActivity.groups.size() == 0) {
						...
						id = MainActivity.groups.get(
								MainActivity.groups.size() - 1).getId() + 1;
					}
				}
				...
				for (int i = 0; i < tl.getChildCount(); i++) {
					CheckBox check = (CheckBox) ((TableRow) tl.getChildAt(i))
							.getChildAt(0);
					if (check.isChecked()) {
				...
				if (update) {
					Util.showToastMessage(context, "Group was updated");
				} else {
					Util.showToastMessage(context, "Group was created");
				}
